...
-   Statistical Transformations
    ...
    -   There are several ways to define the statistical transformations in ggplot2. You can do this within specific geoms, using stat_summary() function, or even by creating another summary df to reference in your ggplot.

# Distributions

Let's get started by creating some distribution plots. We will start with a basic histogram. For this tutorial, we will be using select data from Borrie et al. (2021)'s perceptual learning study. This dataset contains listener intelligibility scores from a pretest phase and posttest phase. The listeners were assigned to one of three speakers with dysarthria: AM1, ALSM1 or PDM10. These are male speakers with dysarthria secondary to cerebellar ataxia, ALS, and Parkinson's disease respectively.
...
borrie_df<- rio::import("Data/Borrie_2021_data.csv")
...
borrie_df_wide <- borrie_df %>%
  tidyr::pivot_wider(names_from = phase,
                     values_from = intel)

# Editing long format

borrie_df_long <- borrie_df %>%
  tidyr::pivot_longer(cols = list_sort_c:pattern_c,
                      names_to = "cognitive_skills",
                      values_to = "standard_score")

rio::export(borrie_df_wide, "Data/Borrie_2021_data_wide.csv")

rio::export(borrie_df_long, "Data/Borrie_2021_long.csv")
...
Ok, so we just made a pretty cool looking plot. But what if we need to make this plot for all our cognitve tests. You could make each plot individually, but an easier solution would to use the facet_wrap() function to create panels for each test. For this, we should use the long df format. Since we want to just model the relationship between these cognitve skills and the pretest intelligibility, we will first filter the long df for just the pretest intelligibility scores.
...
ggsave("Plots/scatter_facet.png", plot = last_plot(), width = 15, height = 10, units = "in", scale = .8)
...
ggsave("Plots/scatter_facet_2.png", plot = last_plot(), width = 15, height = 15, units = "in", scale = .8)
...
ggsave("Plots/scatter_line_point.png", plot = last_plot(), width = 15, height = 10, units = "in", scale = .8)
...
ggsave("Plots/scatter_coord.png", plot = last_plot(), width = 17, height = 10, units = "in", scale = 0.8)

```

##### Fair Warning About Rescaling the Axes Using xlim() and ylim()

There are another set of functions in ggplot2 that can be used to rescale the axes. This is xlim() and ylim()\*. These arguments also ajust the range of the x- and y- axes. However, when you use these functions, it actually will change your data. This can impact the fit of the regression lines, etc. in the figure. You should use coord_cartesian() instead.
...
We will now plot the pretest and posttest intelligibility scores. Since our long df has multiple rows for pretest and posttest values for each listener due to the cogntive test scores, we will use the original df (i.e. the one with two rows per participant).
...
Here are a few more customization options. For these examples, we will be using the pre-post plot example above.
